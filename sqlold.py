...
class SQL:
    def __init__(self):
        self.__usuario="natours_user"
        self.__contrasenia="2411"
        self.__bd="Natours"
        self.__conector=None
        self.conectar()

    def validausuario(self, nombre, contra):
        cursor = self.__conector.cursor(buffered=True)
        query = "select * from empleados where nombre = '" + nombre + "' and contraseña = '" + contra + "'"
    ...
    def conectar(self):
        self.__conector = cnn.connect(user=self.__usuario, password=self.__contrasenia, host="127.0.0.1", database=self.__bd)
    ...
    def insertarempleado(self,listadatos, fechafinal):
        ...
        if fechafinal:
            query = ("insert into empleados( nombre, ap_pat, ap_mat, curp, nss, telefono, id_direccion, "
                     "puesto, acceso, correo, contraseña, sueldo, comision, fecha_inicio, estatus, fecha_final)")
            query+= "values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        else:
            query = ("insert into empleados( nombre, ap_pat, ap_mat, curp, nss, telefono, id_direccion, "
                     "puesto, acceso, correo, contraseña, sueldo, comision, fecha_inicio, estatus)")
    ...
    def editarempleado(self, listadatos, fechafinal):
        cursor = self.__conector.cursor(buffered=True)
        if fechafinal:
            query = ("update empleados set nombre=%s, ap_pat=%s, ap_mat=%s, curp=%s, nss=%s, telefono=%s, id_direccion=%s, "
                     "puesto=%s, acceso=%s, correo=%s, contraseña=%s, sueldo=%s, comision=%s, fecha_inicio=%s, fecha_final=%s, estatus=%s")
        else:
            query = (
                "update empleados set nombre=%s, ap_pat=%s, ap_mat=%s, curp=%s, nss=%s, telefono=%s, id_direccion=%s, "
                "puesto=%s, acceso=%s, correo=%s, contraseña=%s, sueldo=%s, comision=%s, fecha_inicio=%s, estatus=%s")
    ...
    def editarcp(self, listadatos):
        cursor = self.__conector.cursor(buffered=True)
        query = ("update cp set cp=%s, colonia=%s, ciudad=%s, estado=%s, id_pais=%s")
        query += "where id_cp= %s"
        cursor.execute(query, listadatos)
        self.__conector.commit()
    ...
    def editarcliente(self,listadatos):
        cursor = self.__conector.cursor(buffered=True)
        query = ("update clientes set nombre=%s, ap_pat=%s, ap_mat=%s, curp=%s, telefono=%s, nom_empresa=%s, "
                 "id_direccion=%s, correo=%s")
    ...
    def editardireccion(self, listadatos):
        cursor = self.__conector.cursor(buffered=True)
        query = ("update direcciones set calle=%s, no_exterior=%s, no_interior=%s, id_cp=%s")
        query += " where id_direccion=%s"
        cursor.execute(query, listadatos)
        self.__conector.commit()

    def editarprospecto(self, listadatos):
        cursor = self.__conector.cursor(buffered=True)
        query = ("update prospectos set empresa=%s, telefono=%s, nombre=%s, descripcion=%s, id_direccion=%s, "
                 "correo=%s, ap_pat=%s,ap_mat=%s")
        query += " where id_prospecto=%s"
        cursor.execute(query, listadatos)
        self.__conector.commit()
    ...
    def editarservicio(self, listadatos, final):
        cursor = self.__conector.cursor(buffered=True)
        if final:
            query = ("update servicios set tipo_servicio=%s, costo=%s, detalles=%s, titulo=%s, comision=%s, fecha_inicio=%s,"
                     "restricciones=%s, subtipo=%s, ciudad=%s, estado=%s, pais=%s, fecha_final=%s")
            query += " where id_servicio=%s"
        else:
            query = (
                "update servicios set tipo_servicio=%s, costo=%s, detalles=%s, titulo=%s, comision=%s, fecha_inicio=%s,"
                "restricciones=%s, subtipo=%s, ciudad=%s, estado=%s, pais=%s")
            query += " where id_servicio=%s"
        cursor.execute(query, listadatos)
        self.__conector.commit()
    ...
    def editarfactura(self, listadatos):
        cursor = self.__conector.cursor(buffered=True)
        query = "update facturas set status=%s"
